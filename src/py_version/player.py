""" player object that hold stats and RNG """
...
@dataclass
class Player:
    """ hold the imventory, money, posision and name of a player """
    name_of_player: str
    player_id: int
    ...
    def dice_roll(self):
        ...
        print(
            '\n',
            self.name_of_player.capitalize(),
            ' throws the dices ', roll_results[0], ' and ', roll_results[1],
            '. You rolled a total of ', sum(roll_results),
            '\n'
        )
        # if self.player_id == 1:
    ...
    def move_player(self, board):
        ...
        print(
            self.name_of_player.capitalize(),
            ' moves to ',
            board.locations_with_index[new_position]
        )
        return new_position

    def perform_action(self, board, new_position):
        """ call the board everytime the player got on a tile to call the function"""
